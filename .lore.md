...
* **Blog FAQ schemas for AEO**: Blog FAQ and HowTo schemas for AEO/GEO: Structured data for Answer/Generative Engine Optimization. Blog FAQs live in \`src/lib/blog-faqs.ts\` as \`Record\<string, BlogFAQ\[]>\` mapping slug to Q\&A pairs. Blog HowTos follow the same pattern in \`src/lib/blog-howtos.ts\`. The \`blog/\[slug]/page.tsx\` imports both and renders FAQPage/HowTo schemas alongside BlogPosting when data exists. Add 3-5 FAQ entries targeting natural language questions for each new post; add HowTo entries for procedural content. Both use concise, data-rich answers optimized for AI citation.
